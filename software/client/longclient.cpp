...
//int begin_long_test = get_usec_time();
//    while (get_usec_time() - begin_long_test < 1000000 * 60 * 2) { // 2 minutes
    global.startTime = get_usec_time();
...
int main(int argc, char* argv[]) {
    ...
    int numEntries = N * (argc-1);


    std::cout << "Quick Stats:" << std::endl;

    std::cout << "Total Runtime of Test: " <<(global.endTime - global.startTime)/1000000 << "s  ("<<(global.endTime - global.startTime) <<"us)"<< std::endl;
    std::cout << "Images Per Second: " << 1000000.0*(numEntries) / ((global.endTime - global.startTime)) << std::endl;
